...
async def async_sqlite_backend():
    """Provide an async SQLite backend for testing.
    """
...
async def test_async_sqlite_backend_set_get(async_sqlite_backend):
    ...
    result = await async_sqlite_backend.get('key1')
...
async def test_async_sqlite_backend_get_nonexistent(async_sqlite_backend):
    ...
    """
    result = await async_sqlite_backend.get('nonexistent')
...
async def test_async_sqlite_backend_get_with_metadata(async_sqlite_backend):
    ...
    """
    await async_sqlite_backend.set('key1', 'value1', 300)
    value, created_at = await async_sqlite_backend.get_with_metadata('key1')
    assert value == 'value1'
...
async def test_async_sqlite_backend_delete(async_sqlite_backend):
    ...
    """
    await async_sqlite_backend.set('key1', 'value1', 300)
    await async_sqlite_backend.delete('key1')
    result = await async_sqlite_backend.get('key1')
    assert result is NO_VALUE
...
async def test_async_sqlite_backend_clear(async_sqlite_backend):
    ...
    count = await async_sqlite_backend.clear()
    assert count == 2

    result1 = await async_sqlite_backend.get('key1')
    result2 = await async_sqlite_backend.get('key2')
...
async def test_async_sqlite_backend_clear_pattern(async_sqlite_backend):
    ...
    """
    await async_sqlite_backend.set('user:1', 'value1', 300)
    await async_sqlite_backend.set('user:2', 'value2', 300)
    await async_sqlite_backend.set('other:1', 'value3', 300)

    count = await async_sqlite_backend.clear('user:*')
    assert count == 2

    result1 = await async_sqlite_backend.get('user:1')
    result2 = await async_sqlite_backend.get('user:2')
    result3 = await async_sqlite_backend.get('other:1')
...
async def test_async_sqlite_backend_count(async_sqlite_backend):
    ...
    count = await async_sqlite_backend.count()
    assert count == 2
...
async def test_async_sqlite_backend_count_pattern(async_sqlite_backend):
    ...
    count = await async_sqlite_backend.count('user:*')
    assert count == 2
...
async def test_async_file_cache_decorator(temp_cache_dir):
    ...
    @cachu.async_cache(ttl=300, backend='file')
